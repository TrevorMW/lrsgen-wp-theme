...
gulp.task('indvScripts', function() {
    return gulp.src(jsFiles)
        //.pipe( uglify().on( 'error', function(e){ console.log(e); }) )
        .pipe(gulp.dest(jsDest));
});
...
gulp.task('compile', ['coreSass', 'themeSass', 'scripts', 'indvScripts']);

// WATCH FOR CHANGES AND RUN BUNDLED TASKS
gulp.task('watch', function() {
    gulp.watch('./assets/scss/**/*.scss', ['sass']);
    gulp.watch('./assets/js/core/**/*.js', ['scripts', 'indvScripts']);
    gulp.watch('./assets/js/**/*.js', ['indvScripts']);
});